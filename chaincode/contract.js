...
class CertnetContract extends Contract {
    ...
    async createStudent(ctx, studentId, name, email) {
        ...
        let student= this.getStudent(ctx,studentId);
        if(student.length !== 0){
            throw new Error('Stduent id already exists'+ studentId);
        }
    ...
    async issueCertificate(ctx, studentId, courseId, gradeReceived, originalHash) {
        ...
        let certificate = await ctx.stub
                ...
                .catch(err => console.log(err));
        //if(msgSender !== peer0.iit.certification-network.com){
            //throw new Error('Invalid issuer of certificate'+ msgSender);
        //}
